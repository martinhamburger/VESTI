...
## 2026-03 content package addendum

Knowledge Export should now be read with four extra upstream assumptions:

- title comes from app-shell metadata, not from the largest body heading
- body input is `canonical_plain_text`, not a catch-all raw transcript blob
- rich fidelity comes from `semantic_ast_v2`
- `citations[]` and `artifacts[]` are sidecars and should be rendered/exported as separate sections

This does **not** mean Knowledge Export must immediately become fully package-aware in runtime.
It means the architecture note must stop assuming:
- citation evidence remains embedded in body text
- artifact clues should still be recovered from body tail text
- body-only inputs are sufficient to represent rich table / math / code messages
