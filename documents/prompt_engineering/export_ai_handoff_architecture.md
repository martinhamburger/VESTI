...
AI Handoff should now assume the following export-facing input boundary:

- handoff title comes from app-shell metadata
- body text is `canonical_plain_text`
- `semantic_ast_v2` is the target truth source for rich table / math / code fidelity
- `citations[]` and `artifacts[]` are sidecars, not body suffixes

This matters especially for `## Reusable Artifacts` and unresolved execution state:
- citation tails must not be mistaken for grounded evidence
- standalone artifacts must not be flattened into body text just to survive export
